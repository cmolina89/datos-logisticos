...
export const createMockBreakpoint = (
  breakpoint: 'xs' | 'sm' | 'md' | 'lg' | 'xl' | 'xxl' = 'md'
) => {
  const breakpoints = {
...
export const expectToBeAccessible = async (container: HTMLElement) => {
  try {
    // @ts-ignore - jest-axe no tiene tipos oficiales
...
export const expectOpenGraphTag = (property: string, content: string) => {
  const ogTag = document.querySelector(`meta[property="${property}"]`) as HTMLMetaElement
