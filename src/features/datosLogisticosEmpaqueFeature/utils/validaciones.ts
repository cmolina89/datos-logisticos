...
import type { EmpaquesFieldConfig } from '../api/empaquesDynamicConfigService'
...
function validarDimensionConLimite(
...
export function validarEmpaquesProducto(
  ...
  const getReglaCampo = (field: string): { visible: boolean; required: boolean } => {
    const config = fieldsConfig.find(
      item =>
        item.field === field &&
        (!item.appliesTo || item.appliesTo === 'all' || item.appliesTo === data.cualAplica)
    )
  ...
  const shouldValidate = (field: string): boolean => getReglaCampo(field).visible
  const isRequired = (field: string): boolean => getReglaCampo(field).required

  const validarEnteroConRegla = (
    field: string,
    value: number | null | undefined
  ): string | null => {
    if (!shouldValidate(field)) return null
    if (!isRequired(field) && (value === null || value === undefined)) return null
    return validarEnteroPositivo(value)
  }

  const validarNumeroConRegla = (
    field: string,
    value: number | null | undefined
  ): string | null => {
    if (!shouldValidate(field)) return null
    if (!isRequired(field) && (value === null || value === undefined)) return null
    return validarNumeroPositivoConDecimales(value)
  }

  const validarDimensionConRegla = (
    field: string,
    value: number | null | undefined,
    maxVal: number,
    maxKey: string
    ...
    if (!shouldValidate(field)) return null
    if (!isRequired(field) && (value === null || value === undefined)) return null
    return validarDimensionConLimite(value, maxVal, maxKey)
  }

  asignarError(
    errors,
    'cantidadUdsCartonMaster',
    validarEnteroConRegla('cantidadUdsCartonMaster', data.cantidadUdsCartonMaster)
  )
  asignarError(
    errors,
    'multiploCartonMaster',
    validarEnteroConRegla('multiploCartonMaster', data.multiploCartonMaster)
  )

  if (shouldValidate('unidadPeso') && isRequired('unidadPeso') && !data.unidadPeso?.trim()) {
    errors.unidadPeso = KEY_REQUERIDO
  }
  asignarError(errors, 'peso', validarNumeroConRegla('peso', data.peso))

  if (shouldValidate('unidadMedida') && isRequired('unidadMedida') && !data.unidadMedida?.trim()) {
    errors.unidadMedida = KEY_REQUERIDO
  }
  asignarError(errors, 'alto', validarDimensionConRegla('alto', data.alto, 500, KEY_ALTO_MAX))
  asignarError(
    errors,
    'frente',
    validarDimensionConRegla('frente', data.frente, 500, KEY_FRENTE_MAX)
  )
  asignarError(errors, 'fondo', validarDimensionConRegla('fondo', data.fondo, 500, KEY_FONDO_MAX))
