...
import type { EmpaquesFieldConfig, EmpaquesModalConfig } from '../api/empaquesDynamicConfigService'
...
interface TarjetaEmpaquesProductoProps {
  ...
  modalConfig?: EmpaquesModalConfig | null
  fieldsConfig?: EmpaquesFieldConfig[]
  dynamicConfigLoading?: boolean
  dynamicConfigError?: string | null
}
...
const getDefaultCampoRule = (): CampoUIRule => ({
  visible: true,
  required: true,
  editable: true,
})

const resolveCampoRule = (
  field: string,
  fieldsConfig: EmpaquesFieldConfig[],
  cualAplica: CualAplicaEmpaque
): CampoUIRule => {
  const defaultRule = getDefaultCampoRule()
  const config = fieldsConfig.find(
    item =>
      item.field === field &&
      (!item.appliesTo || item.appliesTo === 'all' || item.appliesTo === cualAplica)
  )

  if (!config) return defaultRule

  return {
    visible: config.visible ?? defaultRule.visible,
    required: config.required ?? defaultRule.required,
    editable: config.editable ?? defaultRule.editable,
    label: config.label,
...
const TarjetaEmpaquesProducto: React.FC<TarjetaEmpaquesProductoProps> = ({
  ...
  const campoEsRequerido = useCallback(
    (campo: CampoEmpaques) => resolveCampoRule(campo, fieldsConfig, e.cualAplica).required,
    [fieldsConfig, e.cualAplica]
  )
  ...
  const validarCampoInline = useCallback(
    (campo: CampoEmpaques, valor: number | string | null | undefined) => {
      const required = campoEsRequerido(campo)
      const emptyValue = valor == null || (typeof valor === 'string' && valor.trim() === '')
  ...
  const reglaCantidad = resolveCampoRule('cantidadUdsCartonMaster', fieldsConfig, e.cualAplica)
  const reglaMultiplo = resolveCampoRule('multiploCartonMaster', fieldsConfig, e.cualAplica)
  const reglaNumeroPiezas = resolveCampoRule('numeroPiezas', fieldsConfig, e.cualAplica)
  const reglaUnidadPeso = resolveCampoRule('unidadPeso', fieldsConfig, e.cualAplica)
  const reglaPeso = resolveCampoRule('peso', fieldsConfig, e.cualAplica)
  const reglaUnidadMedida = resolveCampoRule('unidadMedida', fieldsConfig, e.cualAplica)
  const reglaAlto = resolveCampoRule('alto', fieldsConfig, e.cualAplica)
  const reglaFrente = resolveCampoRule('frente', fieldsConfig, e.cualAplica)
  const reglaFondo = resolveCampoRule('fondo', fieldsConfig, e.cualAplica)

  const modalTitle = modalConfig?.title || t('datosLogisticos.segmento3.modalCualAplicaTitle')
  const modalSubtitle =
    modalConfig?.subtitle || t('datosLogisticos.segmento3.modalCualAplicaSubtitle')
  const modalUsesTitle =
    modalConfig?.usesTitle || t('datosLogisticos.segmento3.modalCualAplicaUsesTitle')
  const modalExampleTitle =
    modalConfig?.exampleTitle || t('datosLogisticos.segmento3.modalCualAplicaExampleTitle')
    ...
    <div className="segmento-carton-master">
      ...
      >
        <div className="segmento-carton-modal-shell">
          ...
          <div className="segmento-carton-modal-header">
            ...
            <h2 className="segmento-carton-modal-title">{modalTitle}</h2>
            <p className="segmento-carton-modal-subtitle">{modalSubtitle}</p>
          </div>

          <div className="segmento-carton-modal-grid">
            <ModalCard
              title={
                modalConfig?.cartonMaster?.title || t('datosLogisticos.segmento3.cartonMaster')
              }
              usesTitle={modalConfig?.cartonMaster?.usesTitle || modalUsesTitle}
              bullets={
                modalConfig?.cartonMaster?.bullets?.length
                  ? modalConfig.cartonMaster.bullets
                  : [
              ...
              exampleTitle={modalConfig?.cartonMaster?.exampleTitle || modalExampleTitle}
              example={
                modalConfig?.cartonMaster?.example ||
                t('datosLogisticos.segmento3.modalCualAplicaCartonMasterExample')
              }
              actionLabel={
                modalConfig?.cartonMaster?.actionLabel ||
                t('datosLogisticos.segmento3.modalCualAplicaCartonMasterAction')
              }
            ...
            <ModalCard
              title={modalConfig?.bulto?.title || t('datosLogisticos.segmento3.bulto')}
              usesTitle={modalConfig?.bulto?.usesTitle || modalUsesTitle}
              bullets={
                modalConfig?.bulto?.bullets?.length
                  ? modalConfig.bulto.bullets
                  : [
              ...
              exampleTitle={modalConfig?.bulto?.exampleTitle || modalExampleTitle}
              example={
                modalConfig?.bulto?.example ||
                t('datosLogisticos.segmento3.modalCualAplicaBultoExample')
              }
              actionLabel={
                modalConfig?.bulto?.actionLabel ||
                t('datosLogisticos.segmento3.modalCualAplicaBultoAction')
              }
      ...
      {!collapsed && (
        <>
          ...
          {dynamicConfigLoading && <p>Cargando configuracion dinamica de empaques...</p>}
          {dynamicConfigError && <p>{dynamicConfigError}</p>}
          ...
          <div className="segmento-carton-catalogo-referencia">
            ...
            {!leadTimesLoading && !leadTimesError && leadTimesInfoMessage && (
              <p>{leadTimesInfoMessage}</p>
            )}
            {!leadTimesLoading && !leadTimesError && leadTimes && (
              <p>
                <strong>Lead-times:</strong> SKU {leadTimes.sku ?? 'N/D'} · Tránsito{' '}
                {leadTimes.transitTime ?? 'N/D'} · Producción{' '}
                {leadTimes.productionLeadTime ?? 'N/D'} · Suministro{' '}
                {leadTimes.supplyLeadTime ?? 'N/D'}
              </p>
            )}
          </div>
          ...
          <div className="segmento-carton-pregunta p-mb-3">
            ...
            <div className="segmento-radios-cual-aplica">
              ...
              {opcionesCualAplica.length === 0 && (
                <p>No hay opciones configuradas para este apartado.</p>
              )}
            </div>
          </div>
          <br />
              ...
              <div className="segmento-carton-form p-fluid">
                <div className="row pb-2">
                  <div className="col-md-4">
                    {reglaCantidad.visible ? (
                      <CampoNumerico
                        id="cantidadUdsCartonMaster"
                        value={e.cantidadUdsCartonMaster}
                        label={reglaCantidad.label || t('datosLogisticos.segmento3.cantidadUds')}
                        infoText={
                          reglaCantidad.infoText || t('datosLogisticos.segmento3.cantidadUdsInfo')
                        }
                        min={1}
                        max={99999}
                        useGrouping={false}
                        maxFractionDigits={0}
                        required={reglaCantidad.required}
                        disabled={!reglaCantidad.editable}
                        placeholder={
                          reglaCantidad.placeholder ||
                          t('datosLogisticos.segmento3.placeholderCantidadUds')
                        }
                  ...
                  <div className="col-md-4">
                    {reglaMultiplo.visible ? (
                      <CampoNumerico
                        id="multiploCartonMaster"
                        value={e.multiploCartonMaster}
                        label={reglaMultiplo.label || t('datosLogisticos.segmento3.multiplo')}
                        infoText={
                          reglaMultiplo.infoText || t('datosLogisticos.segmento3.multiploInfo')
                        }
                        min={1}
                        max={99999}
                        useGrouping={false}
                        maxFractionDigits={0}
                        required={reglaMultiplo.required}
                        disabled={!reglaMultiplo.editable}
                        placeholder={
                          reglaMultiplo.placeholder ||
                          t('datosLogisticos.segmento3.placeholderMultiplo')
                        }
                  ...
                  <div className="col-md-4">
                    {e.cualAplica === 'bulto' && reglaNumeroPiezas.visible ? (
                      <div className="segmento-carton-campo">
                        <label htmlFor="empaques-numeroPiezas" className="p-block segmento-label">
                          <span className="flex">
                            {reglaNumeroPiezas.label || t('datosLogisticos.segmento3.numeroPiezas')}
                            {reglaNumeroPiezas.required && (
                        ...
                        <InputNumber
                          ...
                          disabled={!reglaNumeroPiezas.editable}
                          placeholder={
                            reglaNumeroPiezas.placeholder ||
                            t('datosLogisticos.segmento3.placeholderNumeroPiezas')
                          }
                ...
                <div className="segmento-carton-fila segmento-carton-fila-3 p-mb-2">
                  {reglaUnidadPeso.visible ? (
                    <CampoDropdown
                      id="empaques-unidadPeso"
                      value={e.unidadPeso}
                      label={reglaUnidadPeso.label || t('datosLogisticos.segmento2.unidadPeso')}
                      required={reglaUnidadPeso.required}
                      disabled={!reglaUnidadPeso.editable}
                      placeholder={
                        reglaUnidadPeso.placeholder ||
                        t('datosLogisticos.segmento2.placeholderUnidadPeso')
                      }
                  ...
                  {reglaPeso.visible ? (
                    <CampoNumerico
                      id="empaques-peso"
                      value={e.peso}
                      label={reglaPeso.label || t('datosLogisticos.segmento2.peso')}
                      required={reglaPeso.required}
                      disabled={!reglaPeso.editable}
                      infoText={reglaPeso.infoText}
                      min={0.01}
                      maxFractionDigits={2}
                      placeholder={
                        reglaPeso.placeholder || t('datosLogisticos.segmento2.placeholderPeso')
                      }
                      error={errors.peso}
                ...
                <div className="segmento-carton-fila segmento-carton-fila-3 p-mb-2">
                  {reglaUnidadMedida.visible ? (
                    <CampoDropdown
                      id="empaques-unidadMedida"
                      value={e.unidadMedida}
                      label={reglaUnidadMedida.label || t('datosLogisticos.segmento2.unidadMedida')}
                      required={reglaUnidadMedida.required}
                      disabled={!reglaUnidadMedida.editable}
                      placeholder={
                        reglaUnidadMedida.placeholder ||
                        t('datosLogisticos.segmento2.placeholderUnidadMedida')
                      }
                      options={
                        opcionesUnidadMedidaDimensiones as Array<{
                          value: string
                          label: string
                        }>
                      }
                  ...
                  {reglaAlto.visible ? (
                    <CampoNumerico
                      id="empaques-alto"
                      value={e.alto}
                      label={reglaAlto.label || t('datosLogisticos.segmento2.alto')}
                      required={reglaAlto.required}
                      disabled={!reglaAlto.editable}
                      infoText={reglaAlto.infoText}
                      min={0.01}
                      max={500}
                      maxFractionDigits={2}
                      placeholder={
                        reglaAlto.placeholder || t('datosLogisticos.segmento2.placeholderAlto')
                      }
                      error={errors.alto}
                  ...
                  {reglaFrente.visible ? (
                    <CampoNumerico
                      id="empaques-frente"
                      value={e.frente}
                      label={reglaFrente.label || t('datosLogisticos.segmento2.frente')}
                      required={reglaFrente.required}
                      disabled={!reglaFrente.editable}
                      infoText={reglaFrente.infoText}
                      min={0.01}
                      max={500}
                      maxFractionDigits={2}
                      placeholder={
                        reglaFrente.placeholder || t('datosLogisticos.segmento2.placeholderFrente')
                      }
                      error={errors.frente}
                ...
                <div className="segmento-carton-fila segmento-carton-fila-3 p-mb-2">
                  {reglaFondo.visible ? (
                    <CampoNumerico
                      id="empaques-fondo"
                      value={e.fondo}
                      label={reglaFondo.label || t('datosLogisticos.segmento2.fondo')}
                      required={reglaFondo.required}
                      disabled={!reglaFondo.editable}
                      infoText={reglaFondo.infoText}
                      min={0.01}
                      max={500}
                      maxFractionDigits={2}
                      placeholder={
                        reglaFondo.placeholder || t('datosLogisticos.segmento2.placeholderFondo')
                      }
                      error={errors.fondo}
