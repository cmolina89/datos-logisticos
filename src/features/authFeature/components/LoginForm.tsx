...
const LoginForm: React.FC<LoginFormProps> = ({ onLogin, isLoading = false }) => {
    // S1854 fix: se eliminó `const { t } = useTranslation()` — no se usa en este componente
    const [email, setEmail] = useState('');
    // SECURITY FIX: Usar undefined en lugar de cadena vacía para evitar detección de campos vacíos
    const [password, setPassword] = useState<string | undefined>(undefined);
    // S1854 fix: se eliminaron isEmailInvalid e isPasswordInvalid — se asignaban pero nunca se leían en el JSX
    const emailId = useId();
